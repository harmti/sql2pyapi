...
from sql2pyapi.sql_models import SQLParameter, ReturnColumn, ParsedFunction
...
parse_params_test_cases = [
    ...
    ("p_id integer, p_email varchar", [
        ("p_id", "id", "integer", "int", False),
        ("p_email", "email", "varchar", "str", False)
    ], set()),
...
return_clause_test_cases = [
    ...
    ("TABLE(id integer, name text)", True, False, True, "TestReturnsResult", [
        ("id", "integer", "Optional[int]", True),
        ("name", "text", "Optional[str]", True)
    ], None, {"dataclass", "Optional"}),
    # Return SETOF TABLE - parser treats this as returns_table=True
    ("SETOF TABLE(id uuid, active boolean)", True, False, True, "List[TestReturnsResult]", [
...
@pytest.mark.parametrize("return_clause, returns_table, returns_record, returns_setof, "
                         "expected_return_type, expected_columns, expected_setof_table, expected_imports", 
                         return_clause_test_cases)
def test_return_clause_via_public_api(return_clause, returns_table, returns_record, returns_setof,
                                    expected_return_type, expected_columns, expected_setof_table, expected_imports):
    ...
    if "dataclass" in expected_imports:
        ...
        if expected_return_type == "TestReturnsResult" and func.return_type.startswith("List["):
            assert "TestReturnsResult" in func.return_type, f"Expected class name 'TestReturnsResult' in {func.return_type}"
    ...
    if expected_setof_table:
        assert func.setof_table_name == expected_setof_table, f"Expected setof_table_name={expected_setof_table}, got {func.setof_table_name}"
    
    # Verify return columns if applicable
    if expected_columns:
        assert len(func.return_columns) == len(expected_columns), f"Expected {len(expected_columns)} columns, got {len(func.return_columns)}"
        for i, (name, sql_type, py_type, is_optional) in enumerate(expected_columns):
            assert func.return_columns[i].name == name, f"Expected column name {name}, got {func.return_columns[i].name}"
            assert func.return_columns[i].sql_type == sql_type, f"Expected SQL type {sql_type}, got {func.return_columns[i].sql_type}"
            assert func.return_columns[i].python_type == py_type, f"Expected Python type {py_type}, got {func.return_columns[i].python_type}"
            assert func.return_columns[i].is_optional == is_optional, f"Expected is_optional={is_optional}, got {func.return_columns[i].is_optional}"
...
def test_schema_qualified_table_integration():
    ...
    table_sql = create_test_table("public.companies", """
...
def test_enum_type_integration():
    ...
    func_sql = create_test_function(
        ...
        "TABLE(user_id uuid, status status_type)"
    )
