...
async def create_user(conn: AsyncConnection, clerk_id: str, email: Optional[str] = None, email_verified: Optional[bool] = None, first_name: Optional[str] = None, last_name: Optional[str] = None) -> List[CreateUserResult]:
    ...
    async with conn.cursor() as cur:
        ...
        if not rows:
            return []
        # Assuming list of tuples for SETOF composite type CreateUserResult
        try:
            ...
            try:
                ...
                return [CreateUserResult(**row_dict) for row_dict in processed_rows]
            except Exception as e:
                # Failed to map rows to dataclass list for CreateUserResult - Error: {e}
...
async def get_user_by_clerk_id(conn: AsyncConnection, clerk_id: str) -> List[User]:
    ...
    async with conn.cursor() as cur:
        ...
        if not rows:
            return []
        # Assuming list of tuples for SETOF composite type User
        try:
            ...
            try:
                ...
                # Failed to map rows to dataclass list for User - Error: {e}
                return [] # Or raise error?
...
async def create_company(conn: AsyncConnection, name: str, industry: str, size: str, primary_address: str, user_id: UUID) -> List[Company]:
    ...
    async with conn.cursor() as cur:
        ...
        # Assuming simple tuple return for composite type Company
        try:
            ...
            try:
                ...
                if all(value is None for value in row_dict.values()):
                    return []
                instance = Company(**row_dict)
                return [instance]
            except Exception as e:
                # Failed to map row to dataclass Company: {row} - Error: {e}
...
async def list_user_companies(conn: AsyncConnection, user_id: UUID) -> List[Company]:
    ...
    async with conn.cursor() as cur:
        ...
        if not rows:
            return []
        # Assuming list of tuples for SETOF composite type Company
        try:
            ...
            try:
                ...
                # Failed to map rows to dataclass list for Company - Error: {e}
                return [] # Or raise error?

async def update_company(conn: AsyncConnection, company_id: UUID, user_id: UUID, name: Optional[str] = None, industry: Optional[str] = None, size: Optional[str] = None, primary_address: Optional[str] = None) -> List[Company]:
