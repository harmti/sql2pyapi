...
async def get_user_with_enums_index_access(conn: psycopg.AsyncConnection, user_id: int) -> UserWithEnums | None:
...
async def get_user_with_enums_tuple_unpacking(conn: psycopg.AsyncConnection, user_id: int) -> UserWithEnums | None:
...
async def test_dict_row_with_index_access_fails():
    ...
    mock_cursor.description = [("user_id",), ("username",), ("status",), ("role",)]

    # Simulate fetchone returning a dictionary (as dict_row would)
    mock_dict_row = {"user_id": 1, "username": "admin_user", "status": "active", "role": "admin"}
...
@pytest.mark.asyncio
async def test_tuple_unpacking_catches_type_error():
    ...
    # Patch the UserWithEnums.__init__ to raise TypeError when called with *row
    # This simulates what would happen when trying to unpack a dict
    with patch.object(UserWithEnums, "__init__", side_effect=TypeError("cannot unpack mapping")):
        # Call the function with tuple unpacking - should catch and re-raise TypeError
