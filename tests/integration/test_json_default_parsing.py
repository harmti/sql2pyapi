...
def test_json_default_value_parsing():
    ...
    # Get the table
    table_name = list(parser.table_schemas.keys())[0]
    ...
    assert len(table_columns) == 2, f"Expected 2 columns, got {len(table_columns)}: {[col.name for col in table_columns]}"
...
def test_json_default_value_parsing_multiple_nested():
    ...
    assert len(table_columns) == 3, f"Expected 3 columns, got {len(table_columns)}: {[col.name for col in table_columns]}"
