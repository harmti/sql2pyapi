...
"""
    ...
    async def test_function():
        ...
        result = await upsert_metering_point_from_discovery(
            mock_conn,
        ...
        assert isinstance(result, MeteringPointUpsertResult)
        assert isinstance(result.metering_point, MeteringPoint), f"Expected MeteringPoint, got {type(result.metering_point)}"
        assert result.metering_point.ean == "123456789"
        assert result.was_created is True
