...
from ..comment_parser import find_preceding_comment
...
from .utils import generate_dataclass_name
from .utils import sanitize_for_class_name
...
class SQLParser:
    ...
    def _map_sql_to_python_type(
        self, sql_type: str, is_optional: bool = False, context: str | None = None
    ) -> tuple[str, set[str]]:
        """
    ...
    def _parse_column_definitions(self, col_defs_str: str, context: str | None = None) -> tuple[list[ReturnColumn], set[str]]:
        ...
        return parse_column_definitions(
            col_defs_str, context, self.enum_types, self.table_schemas, self.composite_types
        )
    ...
    def _parse_create_table(self, sql_content: str):
        ...
        table_schemas, table_schema_imports = parse_create_table(
            sql_content, self.table_schemas, self.table_schema_imports, self.enum_types, self.composite_types
        )
    ...
    def _parse_create_type(self, sql_content: str):
        ...
        composite_types, composite_type_imports = parse_create_type(
            sql_content, self.composite_types, self.composite_type_imports, self.enum_types, self.table_schemas
        )
    ...
    def _parse_params(self, param_str: str, context: str | None = None) -> tuple[list[SQLParameter], set[str]]:
        """
    ...
    def _parse_return_clause(
        self, match_dict: dict, initial_imports: set[str], function_name: str | None = None
    ) -> tuple[dict, set[str]]:
        """
    ...
    def _parse_record_function_body(self, function_body: str, function_name: str) -> list[ReturnColumn] | None:
        """
    ...
    def _parse_select_columns(self, select_list: str, function_name: str) -> list[ReturnColumn]:
        ...
        for _i, part in enumerate(column_parts):
            part = part.strip()
            if not part:
                continue
    ...
    def _analyze_select_column(self, column_expr: str, function_name: str) -> tuple[str | None, str | None, bool]:
        """
    ...
    def _infer_column_type(self, column_name: str, function_name: str) -> tuple[str, bool]:
        ...
        for _table_name, columns in self.table_schemas.items():
            for col in columns:
                if col.name == column_name:
                    return col.python_type, col.is_optional

        # Search through composite types
        for _type_name, columns in self.composite_types.items():
            for col in columns:
                if col.name == column_name:
                    return col.python_type, col.is_optional
    ...
    def parse(
    ...
    ) -> tuple[list[ParsedFunction], dict[str, set[str]], dict[str, list[ReturnColumn]]]:
        """
        ...
        if schema_content:
            try:
                ...
                logging.exception(f"Error parsing schema content: {e}")
        ...
        logging.debug(
            f"--- SQL Content after comment removal: ---\n{sql_no_comments[:200]}...\n--------------------------------------------"
        )
        ...
        for _i, match in enumerate(match_list):
            ...
            try:
                ...
                if original_start_byte == -1:
                    logging.warning(
                        f"Could not find exact function start for {sql_name} near estimate line {approx_line_in_stripped}. Searching from start."
                    )
                ...
                if original_start_byte != -1:
                    ...
                    logging.error(
                        f"CRITICAL: Cannot find function definition start for '{sql_name}' in original SQL. Comment association may be wrong."
                    )
                ...
                logging.debug(
                    f"Checking RECORD parsing for '{sql_name}': returns_record={return_info.get('returns_record')}, in_body_dict={sql_name in body_match_dict}"
                )
                if return_info.get("returns_record") and sql_name in body_match_dict:
                    ...
                    if function_body:
                        ...
                        if record_columns:
                            ...
                            logging.warning(
                                f"Could not parse RECORD structure for function '{sql_name}', keeping original Tuple behavior"
                            )
                ...
                if base_py_type != "None":
                        ...
                        if base_py_type == "DataclassPlaceholder":
                            ...
                            if return_info.get("returns_sql_type_name"):
                                ...
                                if return_info["returns_sql_type_name"] in self.enum_types:
                                    ...
                                    enum_name = "".join(
                                        word.capitalize() for word in return_info["returns_sql_type_name"].split("_")
                                    )
                    ...
                    elif return_info.get("returns_record") and return_info.get("return_columns"):
                    ...
                    elif return_info.get("returns_enum_type"):
                ...
                logging.exception(f"Error parsing function {sql_name if sql_name else 'UNKNOWN'}: {e!s}")
                import traceback
        ...
        for func in functions:
            if func.returns_setof and func.setof_table_name:
                ...
                if table_name in self.table_schemas and table_name not in composite_types_to_return:
                    logging.debug(
                        f"Adding schema for table '{table_name}' used in SETOF return to composite types for dataclass generation."
                    )
                    ...
                    if self.fail_on_missing_schema:
                        raise ValueError(
                            f"Function '{func.sql_name}' returns SETOF '{table_name}', but no schema found for this table/type. 'fail_on_missing_schema' is True."
                        )
                    else:
                        logging.warning(
                            f"Function '{func.sql_name}' returns SETOF '{table_name}', but no schema found for this table/type. 'fail_on_missing_schema' is False."
                        )
            ...
            if current_type in composite_types_to_return:
                ...
                for column in columns:
                    ...
                    if sql_type in self.table_schemas and sql_type not in composite_types_to_return:
                        logging.debug(
                            f"Adding referenced table type '{sql_type}' to composite types for dataclass generation."
                        )
...
def parse_sql(
    sql_content: str, schema_content: str | None = None, fail_on_missing_schema: bool = False
) -> tuple[list[ParsedFunction], dict[str, set], dict[str, list[ReturnColumn]], dict[str, list[str]]]:
    """
