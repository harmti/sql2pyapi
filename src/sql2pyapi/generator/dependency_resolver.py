...
def extract_class_references(python_type: str) -> set[str]:
    ...
    class_refs = {cls for cls in potential_classes if cls not in builtin_types}

    return class_refs


def analyze_dataclass_dependencies(dataclasses: dict[str, list[ReturnColumn]]) -> dict[str, set[str]]:
    """
        ...
        for column in columns:
            ...
            for ref in refs:
                if ref in dataclasses and ref != class_name:
                    ...
                    logging.debug(
                        f"Dependency found: {class_name} -> {ref} (field: {column.name}: {column.python_type})"
                    )

    return dict(dependencies)


def topological_sort(nodes: set[str], dependencies: dict[str, set[str]]) -> tuple[list[str], list[str] | None]:
    """
...
def collect_referenced_table_types(
    custom_types: dict[str, list[ReturnColumn]], table_schemas: dict[str, list[ReturnColumn]]
...
def resolve_dataclass_order(
    custom_types: dict[str, list[ReturnColumn]], table_schemas: dict[str, list[ReturnColumn]] | None = None
    ...
    all_types = collect_referenced_table_types(custom_types, table_schemas) if table_schemas else custom_types
