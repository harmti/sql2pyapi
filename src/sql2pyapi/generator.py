...
def generate_python_code(
    ...
    # import_statements = present_standard_imports + other_imports # REMOVE THIS LINE or comment out

    # --- Assemble code --- REVISED ---
    # Assemble the final code string
    auto_gen_comment = f"# Auto-generated by sql2pyapi from {os.path.basename(source_sql_file)}" if source_sql_file else "# Auto-generated by sql2pyapi"
    header = f"""# -*- coding: utf-8 -*-
{auto_gen_comment}"""

    # Use the raw collected imports directly and sort them
    imports_section = "\n".join(sorted(list(current_imports)))
    # dataclasses_section and functions_section are already joined correctly above
    dataclasses_section = "\n\n".join(dataclasses_section_list)
    functions_section = "\n\n".join(generated_functions)

    # Build the final code string with controlled spacing
    code_parts = [header]
    if imports_section:
        code_parts.append(imports_section)
    if dataclasses_section:
        code_parts.append(dataclasses_section)
